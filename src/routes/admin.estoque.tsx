...
const TIPO_LABELS: Record<string, { label: string; icon: React.ReactNode }> = {
  entrada: { label: "Entrada", icon: <ArrowUp className="h-3 w-3 text-foreground" /> },
  saida: { label: "Saída", icon: <ArrowDown className="h-3 w-3 text-destructive" /> },
  reserva: { label: "Reserva", icon: <Lock className="h-3 w-3 text-muted-foreground" /> },
  estorno_reserva: { label: "Estorno", icon: <Undo2 className="h-3 w-3 text-muted-foreground" /> },
  ajuste: { label: "Ajuste", icon: <ArrowUp className="h-3 w-3" /> },
};
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button><ArrowUp className="mr-1 h-4 w-4" /> Registrar entrada</Button>
          </DialogTrigger>
          <EntradaDialog produtos={produtos ?? []} onClose={() => setOpen(false)} />
        </Dialog>
      </div>

      <h2 className="mb-2 mt-6 text-xs uppercase tracking-widest text-muted-foreground">Histórico de movimentações</h2>
      {isLoading ? (
    ...
    <DialogContent>
      ...
      <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); mut.mutate(); }}>
        ...
        <div>
          ...
          <Input value={obs} onChange={(e) => setObs(e.target.value)} placeholder="Ex: doação, compra, ajuste" />
        </div>
        <DialogFooter>
          <Button type="submit" disabled={!produtoId || mut.isPending}>{mut.isPending ? "Salvando..." : "Registrar"}</Button>
        </DialogFooter>
      </form>
    </DialogContent>
  );
