...
export async function gerarDocumentoSaidaPDF(numero: string): Promise<Uint8Array> {
  ...
  if (!doc || !doc.pedidos) throw new Error("Documento não encontrado");
  ...
  page.drawText("Fundo Bíblico - CCI", { x, y, size: 18, font: bold, color: black });
  y -= 22;
  page.drawText("Documento de Saída", { x, y, size: 11, font, color: gray });

  y -= 30;
  page.drawText(`Nº ${doc.numero}`, { x, y, size: 14, font: bold, color: black });
  page.drawText(
    `Emitido em ${new Date(doc.created_at).toLocaleString("pt-BR")}`,
  ...
  if (pedido.observacao) {
    y -= 30;
    page.drawText("Observação:", { x, y, size: 10, font: bold, color: black });
    y -= 14;
  ...
  page.drawText("Entregue por", { x, y: y - 14, size: 9, font, color: gray });

  page.drawLine({ start: { x: 310, y }, end: { x: 545, y }, thickness: 0.5, color: black });
  page.drawText("Recebido por", { x: 310, y: y - 14, size: 9, font, color: gray });
